...
If you have a machine with enough resources, you can experiment locally without using any Cloud resources. To do that, you first have to install [Ollama](https://ollama.com) and then run the following commands to download the models on your machine:
...
After that you have to get the project code:
...
Then, go to the `src` subfolder and execute the following command lines:
...
> In addition, make sure you [Set up the AWS CLI](https://docs.aws.amazon.com/cli/latest/userguide/getting-started-quickstart.html) using AWS IAM User or Role having sufficient AWS IAM permissions to invoke the Amazon Bedrock Foundation Model of your choice.

### Deploy the sample to AWS

#### AWS prerequisites

- **AWS account**. If you're new to AWS, [get an AWS account](https://aws.amazon.com/resources/create-account/).
...
⚠️ To avoid unnecessary costs, remember to delete ALL your AWS resources if it's no longer in use.

#### Deploy the sample

1. Open a terminal and navigate to the `infra` folder.
2. If not already done, [Set up the AWS CLI](https://docs.aws.amazon.com/cli/latest/userguide/getting-started-quickstart.html) using AWS IAM User or Role having sufficient AWS IAM permissions to deploy the sample.
...
- [Generative AI on AWS](https://www.generativeaionaws.com/)
